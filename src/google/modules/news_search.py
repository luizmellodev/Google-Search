...
def search(query, pages=1, lang='en', area='com', ncr=False, void=True, time_period=False, sort_by_date=False, first_page=0):
    ...
    for i in range(first_page, first_page + pages):
        url = _get_search_url(query, i, lang=lang, area=area, ncr=ncr, time_period=time_period, sort_by_date=sort_by_date).replace('&q=', '&tbm=nws&q=')
        html = get_html(url)

        if html:
            soup = BeautifulSoup(html, "html.parser")
            divs = soup.findAll("div", attrs={"class": "dbsr"})
...
def _get_description(li):
    ...
    sdiv = li.find("div", attrs={"class": "yJHHTd"})
    if sdiv:
        stspan = sdiv.find("div", attrs={"class": "Y3v8qd"})
